...
const groupMessages = {
  get: {
    ...
    description: [
      ...
      "",
      "Every page carries `pinnedMessage` (the room's current active pin summary, or",
      "`null`), `memberReadSeq` (every other active member's read cursor — one entry",
      "per member, `0` for members who have read nothing or who give no receipts, and",
      "an EMPTY object when the caller's own read receipts are off) and",
      "`memberDeliveredSeq` (the same shape for delivery), so the pinned banner and the",
      "sender's per-message ✓ / ✓✓ / ✓✓-blue ticks hydrate without extra calls and",
      "resolve exactly as the inbox row's `lastMessageReadStatus` does.",
    ].join("\n"),
    security: [{ bearerAuth: [] }],
    parameters: [roomIdParam, limitParam, ...seqPaginationParams()],
    responses: {
