...
export class PrivatePinService {
   ...
   * for every participant. Idempotent: null when the message has no active pin
  ...
  async unpinDeletedMessage(
    ...
    if (activePin.pinSystemMessageId) {
      await this.sysMsg
        ...
        .catch((err: unknown) => {
          logger.warn(
            `PrivatePinService|unpinDeletedMessage retract failed: ${String(err)}`
          );
        });
    }
   ...
   * ONE participant. Per-user twin of
   * `PrivateSystemMessageService.retractSystemMessage` (which removes the line
   * for both participants on unpin / delete-for-everyone) — the pin row is
   * untouched, so the peer keeps both. Returns the hidden line's identity for
   * the caller's tombstone, or null when there is nothing to hide.
   */
  ...
  async list(
    ...
    // A message this participant deleted FOR THEMSELVES is gone from their
    // pins too — the pin row stays, so the other participant is unaffected.
    return resolved
  ...
  async getActivePinSummary(
    ...
    // so their pin banner goes with it while the other participant keeps
    // theirs. Read-time, so a reconnect/cold load resolves to the same state.
