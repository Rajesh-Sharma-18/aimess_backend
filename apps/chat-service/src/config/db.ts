...
function safeMongoTarget(): string {
  try {
    ...
    return "(unparseable MONGO_DATABASE_URL)";
  }
}
 ...
 * and crashes the whole service before it can serve anything. Retrying with a
 * capped backoff lets the service ride out that startup window instead of
 * dying, while still failing loudly (with the real error) if Mongo is
 * genuinely misconfigured or unreachable.
 */
