...
export const communityService = {
  ...
  async update(
    ...
    if (nextName && nextName !== previousName) {
      ...
      // The chat-service GeneralRoom.name mirror (which titles community
      // chat-message pushes) is re-synced below via publishCommunityMetaSynced
      // on `changedFields.includes("name")`, so no separate name-changed publish
      // is needed here — that would double-write the same mirror on every rename.
    }
