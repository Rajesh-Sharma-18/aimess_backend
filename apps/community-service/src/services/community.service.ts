...
export const communityService = {
  ...
  async update(
    ...
    if (nextName && nextName !== previousName) {
      ...
      // The chat-side GeneralRoom.name mirror (which titles community
      // chat-message pushes) is refreshed further down by
      // publishCommunityMetaSyncedForChatSafe on `changedFields.includes("name")`.
      // It carries name AND avatar and supersedes the older name-only
      // `community.name_changed` publish that used to live here — emitting both
      // wrote the same mirror twice per rename.
    }
