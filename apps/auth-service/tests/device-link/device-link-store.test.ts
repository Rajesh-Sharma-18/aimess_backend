...
jest.mock("../../src/config/redis.js", () => ({
  redis: { status: "ready", set: setMock },
...
describe("createLinkSession", () => {
  ...
  it("sets the Redis key TTL longer than the logical expiry (sweeper grace window)", async () => {
    ...
    const [, , exFlag, ttlSeconds] = setMock.mock.calls[0] as [
      string,
      string,
      string,
      number,
    ...
    // 60s logical TTL + 30s sweep grace (env defaults) = 90s Redis key TTL —
    // strictly greater than the 60s logical expiresAt so the sweeper can still
    // observe (and atomically claim) the record after it logically expires.
