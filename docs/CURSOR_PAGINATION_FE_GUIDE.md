...
interface ChatMessagePage {       // and ChatInboxPage (items = inbox rows)
...
interface ChatCommunityIncrementalSync {
  data: Array<ChatCommunityMessage & {
...
```ts
async function loadOlder(roomId: string, state: { cursor: number | null; seen: Set<string>; done: boolean }) {
  ...
  state.cursor = pagination.nextCursor != null ? Number(pagination.nextCursor) : null;
