...
| `community:<communityId>` | **auto-joined at connect** for every community the user is an ACTIVE member of (gateway calls `GetUserActiveCommunityIds` gRPC); also joined on explicit `community:join` | community chat events — typing, messages, roster |
...
| `/community` | `community:message:read`          | Messages marked as read                         |
| `/community` | `community:message:reactions:get` | Reactions fetched successfully                  |
| `/community` | `community:message:forward`       | Message forwarded successfully                  |
| `/community` | `community:message:delivered`     | Message delivery receipt recorded               |
| `/community` | `auth:refresh`                    | Token refreshed successfully                    |
...
| `community:updated`   | `user:<id>`        | `{ communityId, roomId, lastMessageId, lastMessage: { contentType, text }, lastMessageAt, senderId, unread }`                                                                                                                                                                                                                                                                                                                                                                                                 | bump-to-top for the community list — fired on every new community message to all active members; **delivered on the `/community` namespace** (not `/chat`) |
...
> **List bump events — `conv:updated` (on `/chat`) / `community:updated` (on `/community`):**
> these are WhatsApp/Telegram-style "move-to-top" hints for the list/inbox
> surface, delivered to `user:<id>` (so a client sitting on the **list** screen —
> joined only `user:<id>`, not inside the conversation — can reorder and update
> the preview **without refetching**). They are **separate concerns on separate
> namespaces**: `conv:updated` (private/group inbox) is delivered on **`/chat`**;
> `community:updated` (community list) is delivered on **`/community`**. A client
> that renders both the chat inbox and the community list must listen on **both**
> namespaces (each socket joins `user:<id>` independently).
...
| `community:message:send`          | yes | `{ communityId, roomId, clientMessageId, message≤4000, contentType, media?:{ files[]≤30 }, location?, contact?, sticker?, parentMessageId? }` — also accepts `contentText` as alias for `message`; accepts top-level `files[]` as alias for `media.files[]` | post a message (ack = `CommunityMessageSendResult`)                                                                                                                            |
...
| `community:message:read`          | yes | `{ communityId, roomId?, upToMessageId }`                                                                                                                                                                                                                   | mark community messages read; triggers broadcast to `community:<id>` + cross-device sync to `user:<readerId>`                                                                  |
| `community:message:reactions:get` | yes | `{ messageId, communityId }`                                                                                                                                                                                                                                | list who reacted to a message                                                                                                                                                  |
| `community:message:forward`       | yes | `{ messageId, communityId, targetCommunityId, targetRoomId?, clientMessageId }`                                                                                                                                                                             | forward message to another community                                                                                                                                           |
| `community:message:delivered`     | yes | `{ communityId, roomId?, upToMessageId }`                                                                                                                                                                                                                   | signal delivery of messages up to this id; broadcasts `community:message:delivered` to the room (sender sees ✓✓)                                                               |
| `auth:refresh`                    | yes | `{ refreshToken }`                                                                                                                                                                                                                                          | refresh access token without reconnect; same as /chat (see §4.1)                                                                                                               |
| `community:typing:start`          | no  | `{ communityId, roomId? }`                                                                                                                                                                                                                                  | broadcast `community:typing:start` to `community:<communityId>`, sender excluded. 6 s server auto-expiry. **New canonical name** (legacy alias `typing:start` still accepted). |
| `community:typing:stop`           | no  | `{ communityId, roomId? }`                                                                                                                                                                                                                                  | broadcast `community:typing:stop` to `community:<communityId>`, sender excluded. **New canonical name** (legacy alias `typing:stop` still accepted).                           |
...
| `community:message:pinned`   | `community:<id>`                                           | `{ communityId, roomId, pin: CommunityMessagePin, pinnedCount: number \| null }` — `pin.originalMessage` is present when the pinned message was deleted: `{ isAvailable: false }`. `CommunityMessagePin = { id, roomId, messageId, pinnedBy, pinnedAt (epoch-ms), messageCreatedAt (epoch-ms), senderId, senderDisplayName, senderAvatar \| null, contentPinned: { text, urls[], files[] }, communityId, pinSystemMessageId \| null, originalMessageDeletedAt (epoch-ms) \| null }`                                                                                                                                                                                              | a message was pinned (or the original message of a pin was deleted)                          |
| `community:message:unpinned` | `community:<id>`                                           | `{ communityId, roomId, messageId, pin: CommunityMessagePin \| null, pinnedCount: number }` — `pin` is the soft-deleted pin record (has `unpinnedAt` / `unpinnedByUserId` set); `null` only on internal error                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | a message was unpinned                                                                       |
...
| `community:added`            | `user:<id>` (the NEW member only)                          | `{ communityId, name, handle, description\|null, avatarUrl (presigned URL)\|null, type:"PUBLIC"\|"PRIVATE", categoryId\|null, categoryName\|null, memberCount, role:"ADMIN"\|"MODERATOR"\|"MEMBER", status:"ACTIVE"\|"CLOSED", via, joinedAt (epoch-ms), addedAt (epoch-ms) }` — **onboarding**: the recipient just became a member (admin add / join-request approve / invite-link redeem / self-join). They are NOT yet in the `community:<id>` room, so `community:member:joined` never reaches them. Full list-row snapshot → **insert the community into the sidebar / "mine" list INSTANTLY** (no GET /communities/mine, no refresh). Idempotent: upsert by `communityId`. | the recipient is added to / joins a community                                                |
...
| `community:member:updated`   | `community:<id>`                                           | `{ communityId, userId, username?, displayName?, avatarUrl?:string\|null, role?:"ADMIN"\|"MODERATOR"\|"MEMBER", updatedAt (epoch-ms number) }` — **roster sync**: a single member's row changed. Emitted on role change (`updateMemberRole`), admin transfer (one event per affected member — incoming ADMIN + outgoing demoted to MEMBER), and profile sync (username/displayName/avatar from user-service)                                                                                                                                                                                                                                                                     | a member's role or profile changed                                                           |
| `community:meta:updated`     | `community:<id>` **and** `user:<id>` (every active member) | `{ communityId, changes:{ name?, avatar?, description?, category?, visibility?, handle? }, community: CommunityMetaDto, updatedAt (epoch-ms number) }` where `CommunityMetaDto = { communityId, name, handle, description, avatar (presigned URL), type, categoryId, categoryName, memberCount, updatedAt }`. **Metadata sync** — apply `community` verbatim to the detail/header and patch the matching list row. DISTINCT from `community:updated` (list bump): a metadata patch, it does **not** reorder. Idempotent on `updatedAt`                                                                                                                                           | community name/avatar/description/category/visibility changed (`update()`)                   |
| `community:message:read`     | `community:<id>`                                           | `{ communityId, readerId, upToMessageId, readAt (epoch-ms) }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | a member marked messages as read                                                             |
| `community:read_sync`        | `user:<readerId>`                                          | `{ communityId, upToMessageId, readAt (epoch-ms) }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | multi-device read sync — your other devices clear unread                                     |
| `community:closed`           | `community:<id>` **and** `user:<id>` (every ex-member)     | `{ communityId, status:"CLOSED", closedAt (epoch-ms number), reason? }` — the owner closed the community; ALL members are auto-removed and it is read-only. Delivered to the room AND to each ex-member's `user:<id>` so clients off the screen still update. **Disable all community actions immediately** (send/react/edit/delete/pin/join) — no refresh, no polling.                                                                                                                                                                                                                                                                                                          | owner closes the community (`POST /communities/:id/close`)                                   |
| `community:reopened`         | `community:<id>` **and** `user:<ownerId>`                  | `{ communityId, status:"ACTIVE", reopenedAt (epoch-ms number) }` — the owner reopened a CLOSED community. Delivered to the room AND the owner's `user:<id>` (the room is empty post-close — every member was evicted — and reopen does not restore the roster, so a room-only emit reaches nobody). Former members are NOT auto-restored; they re-join via the normal flow. Clients may re-enable actions / refresh status from REST.                                                                                                                                                                                                                                            | owner reopens the community (`POST /communities/:id/reopen`)                                 |
| `session:expired`            | (direct to socket)                                         | `{ reason, expiresAt, reconnect, gracePeriod }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | token expiry warning (same shape as /chat); emit `auth:refresh` within `gracePeriod` seconds |
| `community:typing:start`     | `community:<id>` (sender excluded via `socket.to()`)       | `{ eventId (UUID), communityId, roomId (=== communityId), userDetails: { userId, username, displayName, avatarUrl\|null }, timestamp (epoch-ms), userId (deprecated), senderName (deprecated) }` — **delivered to ALL community members in the room regardless of which community screen they have open**; use `typingState[communityId][userDetails.userId]`                                                                                                                                                                                                                                                                                                                    | a member starts typing                                                                       |
| `community:typing:stop`      | `community:<id>` (sender excluded via `socket.to()`)       | Same shape as `community:typing:start` — also emitted on 6 s server TTL, sender disconnect, or member removal/ban. Clear `typingState[communityId][userDetails.userId]`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | a member stops typing                                                                        |

> **Frontend parity note.** The community namespace now mirrors the chat namespace for messaging features where applicable. Use the same event-handling patterns across private chat, group chat, and community chat:
>
> - Read receipts: `message:read` (chat) / `community:message:read` (community)
> - Reaction details: `message:reactions:get` (chat) / `community:message:reactions:get` (community)
> - Forward: `message:forward` (chat) / `community:message:forward` (community)
> - Token refresh: `auth:refresh` works on both `/chat` and `/community` namespaces
> - Multi-device read sync: `read_sync` (chat) / `community:read_sync` (community)
> - Delivery receipts: `message:delivered` (chat) / `community:message:delivered` (community) — community delivery is a broadcast signal (no per-message DB write; delivery state is inferred from membership)
>
> Calls, presence, and friend management remain chat-only (N/A for many-member community rooms).
...
> **Community status (open / closed).** Every community-shaped REST response
> carries `status: "ACTIVE" | "CLOSED"` (owner lifecycle; absent ⇒ ACTIVE).
> `CLOSED` means the owner closed the community — all members are auto-removed,
> the community is read-only, and it can later be reopened. Clients should:
>
> - On `community:closed`: show a closed/read-only state and disable every
>   community action (send, react, edit, delete, pin, join). The user is no
>   longer a member — the community drops off "my communities".
> - On `community:reopened`: the community is open again, but the user is NOT
>   re-joined automatically — they must re-join via the normal flow.
>
> This is separate from `moderationStatus: "ACTIVE" | "SUSPENDED"` (a platform/
> backoffice action that keeps members but blocks writes). Either CLOSED or
> SUSPENDED makes the community non-writable; the backend enforces both centrally.
...
| `community:join_request:update` | an ADMIN/MODERATOR approves or rejects the request **OR** the user cancels their own request                     | `{ communityId, requestId, status:"APPROVED"\|"REJECTED"\|"CANCELLED", communityName, decidedAt (ISO-8601), navigation }` — emitted to the requester's `notify:<userId>`; drives the FE state flip. `APPROVED`/`REJECTED` also deliver an in-app `notification:new`. `CANCELLED` is socket-only (no push — the user initiated it themselves). |
...
| `stream:comment:delete` | yes | `{ streamId, commentId }`                 | delete a comment for all viewers; allowed if requester is the comment author, stream owner, or community ADMIN/MODERATOR; errors → `FORBIDDEN` / `NOT_FOUND`                                                                                                                                               |
...
| `stream:comment:deleted` | `stream:<id>` | `{ commentId, streamId, deletedBy }`                                                                                                 | a comment was deleted (by its author, stream owner, or community ADMIN/MOD); clients must remove it from their local message list by `commentId`                                                                             |
...
```
A sits on the LIST screens: connect /chat AND /community → joins user:<A> on each (no conv:join / community:join)
Someone sends a new message in a chat A belongs to
        chat-service publishes "conv:updated" to user:<A> (+ every other participant)
A: receives "conv:updated" on /chat { type, roomId, lastMessage, lastMessageAt, unread:true }
...
A new community message arrives in a community A is a member of
        chat-service publishes "community:updated" to user:<A> (delivered on /community)
A (listening on /community): receives "community:updated" { communityId, … }
...
| Community multi-device `read_sync` → `community:read_sync` | **shipped** — `community:message:read` broadcasts to `community:<id>`; `community:read_sync` syncs to the reader's own `user:<id>` room (V1 implementation complete)                                                                                                                                                   |
...
`community:message:unpin` · `community:message:read` · `community:message:reactions:get` · `community:message:forward` · `community:message:delivered` ·
`auth:refresh` (`/community`) · `typing:start` (`/community`) · `typing:stop` (`/community`) ·
`notifications:fetch` · `notifications:mark_read`
`community:message:unpin` · `notifications:fetch` · `notifications:mark_read` ·
`stream:join` · `stream:leave` · `stream:comment` · `stream:comment:delete` · `stream:react`

**Server → Client:** `message:new` · `message:edited` · `conv:updated` ·
`chat:catchup:result` · `message:read` ·
...
`community:catchup:result` · `community:member:joined` · `community:message:read` · `community:read_sync` · `community:updated` (`/community`) · `session:expired` (`/community`) · `typing:start` (`/community`) ·
...
`notification:count` · `notification:count_update` ·
`stream:comment:new` · `stream:comment:deleted` · `stream:viewer_count` · `stream:react:new` · `stream:status` · `stream:banned`
...
- a `community:join_request:update` socket event on `/notify`
  (`{ communityId, requestId, status, communityName, decidedAt, navigation }`) for the FE
...
**User self-cancel is also realtime.** When a user cancels their own request
(`DELETE /communities/:id/join-requests/mine` or `DELETE
/communities/:id/join-requests/:requestId`), a `community:join_request:update`
event with `status: "CANCELLED"` is emitted to the requester's `/notify` room.
This syncs the "Requested" → "Join" button flip across all the requester's
connected devices without a page refresh. No push notification is sent (the
user initiated the action themselves).
...
Pin/unpin:          one active pin per room (server-enforced limit). Unpin is a
                    soft-delete (pin record preserved with unpinnedAt). The event
                    carries a full CommunityMessagePin object — replace local pin
                    state verbatim on each community:message:pinned event.
```
