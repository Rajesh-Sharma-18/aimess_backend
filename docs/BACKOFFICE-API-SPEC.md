...
> **Updated (split into three independently-refreshing endpoints):** the dashboard is now served by `GET /dashboard/overview` (`data: { stats }`), `GET /dashboard/charts?period=daily|weekly|monthly` (`data: { activeVsChurned, communitiesGroups }`), and `GET /dashboard/service-status` (`data: { serviceStatus }`). The former merged `/dashboard/stats` endpoint was removed. The `communities-groups` donut now lives inside `/charts`; `period` selects the active-vs-churned granularity. Each endpoint fetches ONLY the upstreams its section needs and is cached independently (~10s).
...
| ------ | --------------------------- | -------------------------------- | ----------------------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | `/dashboard/overview`       | —                                | 🟩 gRPC-live (+stubs)   | ✅ built   | `data: { stats }` — stat cards: totalUsers, newUsersToday, dailyActive, monthlyActive, totalCommunities, totalGroups, bannedUsers (live); totalLivestreams/openReports/churned stubbed. `stale` flags stubbed/degraded fields. Does NOT fetch the series. Cached ~10s.                                                                                                                                                                                                        |
| GET    | `/dashboard/charts`         | `?period=daily\|weekly\|monthly` | 🟩 gRPC-live            | ✅ built   | `data: { activeVsChurned, communitiesGroups }`. Per-day series via auth `GetActiveUserSeries`: daily=15d, weekly=8d, monthly=full month (1st→last day, future days=0). Each bucket `{bucket,dailyActive,monthlyActive,churned}` from `Session.lastActiveAt`. ⚠️ undercounts older days; churn ≈0 until enough history. `communitiesGroups` donut: communities (community-svc) vs groups (chat-svc) + total. Does NOT fetch user/active/banned counts. Cached per-period ~10s. |
| GET    | `/dashboard/service-status` | —                                | 🟪 redis + 🟩 gRPC-live | ✅ built   | `data: { serviceStatus }` — derived from opossum breaker state (open→down, half-open→degraded). Services without a probe → degraded/unknown. Cached ~10s.                                                                                                                                                                                                                                                                                                                     |
| GET    | `/dashboard/quick-links`    | —                                | static/🟨               | ⏳ planned | Counts for the Quick Links panel. Optional — derivable from `/dashboard/overview`.                                                                                                                                                                                                                                                                                                                                                                                            |

### 4.2 User Management (`users.read` / `users.moderate` / `users.delete`)

| Method | Path                      | Perm        | Data source               | Notes                                                                                                                                                                                                                                                                                                                                  |
| ------ | ------------------------- | ----------- | ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | `/users`                  | read        | 🟩 gRPC-live (auth+user)  | List/search users. Filters: `status`, `banned`, `createdAfter`, `q`. Aggregates `AdminListUsers` (auth) + `AdminListProfiles` (user). Each item's `avatarUrl` is a **presigned GET URL** (short-lived; null when no avatar / presign unavailable) and `avatarUrlExpiresIn` is its lifetime in seconds (null when `avatarUrl` is null). |
| GET    | `/users/:id`              | read        | 🟩 gRPC-live              | Full profile: identity (auth) + profile/stats (user) + moderation history (🟦 OWN `ModerationAction`). `profile.avatarUrl` is a **presigned GET URL** (null when none) and `profile.avatarUrlExpiresIn` its lifetime in seconds (null when `avatarUrl` is null).                                                                       |
