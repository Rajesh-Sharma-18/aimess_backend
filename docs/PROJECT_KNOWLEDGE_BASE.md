...
- **2026-06-15 (community chat send/edit `SERVICE_ERROR` — 2 distinct root causes fixed; commits `e2ecbda` + `922ed49`):** QA `pnpm -C testing-suite orchestrate` flagged two `/community` socket failures, both surfacing as the gateway's generic `SERVICE_ERROR` (it wraps every downstream gRPC rejection, so the real exception was in chat-service). **(1) `community:message:edit` — CONSISTENT (commit `e2ecbda`):** `CommunityMessageService.editMessage` guarded `if (message.messageType !== "text")` (lower-case), but community messages are persisted UPPER ("TEXT") — the gRPC `sendCommunityMessage` handler does `(contentType||"TEXT").toUpperCase()`. So **every** community text-message edit threw `CHAT_EDIT_TEXT_ONLY` → gRPC INTERNAL → `SERVICE_ERROR`. Private/group already compared `!== "TEXT"`; community was the lone outlier. Fix: edit guard now `normalizeMessageType(message.messageType) !== "TEXT"` (also tolerates legacy lower-case rows) and `sendMessage` persists `normalizeMessageType(params.messageType)` for parity. **(2) `community:message:send` — INTERMITTENT/load-dependent (commit `922ed49`):** a _different_ root cause — an async room-provisioning race (NOT shared with #1; send has no messageType guard). Community chat rooms (`GeneralRoom`, id===communityId) were provisioned ONLY by chat-service's `CommunityRoomSyncConsumer` on the async `community.created` RabbitMQ event, so a first send racing ahead of the consumer hit `findRoomById`→null→`COMMUNITY_CHAT_DISABLED`→`SERVICE_ERROR` (passed in isolation, failed under full-suite concurrency = QA BUG-002). Fix (sync provisioning, user-chosen): new `EnsureCommunityRoom` RPC on `community.proto` → chat-service `createCommunityImpl.ensureCommunityRoom` → new `CommunityMessageService.provisionRoom` → the same idempotent `generalRoomRepo.provisionForCommunity` upsert the event consumer + boot reconciler use; community-service `community.service.ts` create flow now **awaits** `getChatClient().ensureCommunityRoom(...)` (new method on the existing `grpc/chat.client.ts`, own breaker, no silent fallback) **before** returning, so the room is guaranteed to exist; the async `community.created` event stays a backstop (idempotent upsert) and creation still succeeds (logs + falls back) if chat-service is briefly down. **Verified E2E live** (`pnpm -C testing-suite orchestrate --concurrency 2`, full 45-scenario suite ×3): `community.basic` + `community.reactPinEdit` both GREEN every run (were BUG-002-intermittent + edit-consistent-fail before); `community.basic` gone from `BUG_REPORT.md`. Proto loads clean (25 methods); all 5 changed files `tsc`-clean. (Two unrelated pre-existing `tsc` errors — `chat-service/server.ts:422` PrivateMessageController 4-arg, `community.service.ts:2344` toDiscoverItem 4-arg — are from the concurrent reactions/REST-send feature work, not these changes.)
- **2026-06-15 (chat private-send idempotency race fixed — concurrent `clientMessageId` no longer 500s):** The `chaos.duplicateConcurrent` QA scenario (5 concurrent `message:send` sharing one `clientMessageId`) surfaced a check-then-insert race: the pre-send `findByClientMessageId` dedup in `PrivateMessageService.sendMessage`/`forwardMessage` is not atomic, so concurrent requests all miss it and all insert. The partial-unique index `private_messages_idempotency_idx` (`{roomId,senderId,clientMessageId}`, created via `$runCommandRaw` in `server.ts`, **not** in the Prisma schema) lets exactly one win; the losers' inserts threw E11000/P2002 which was **uncaught** → gRPC `sendMessage` returned `INTERNAL` → gateway mapped it to `SERVICE_ERROR`/`retryable` ("Something went wrong"). **Fix:** wrap the insert in try/catch — on a duplicate-key error, re-read by `clientMessageId` and return the winning row (the idempotent "already sent" collapse), mirroring what **group + community already did**. New shared predicate `apps/chat-service/src/lib/db-errors.ts` `isDuplicateKeyError()` handles Prisma `P2002` **and** raw Mongo `E11000`/`11000`; group + community `sendMessage` refactored onto it too (they previously checked only `P2002`, so they now also gain the raw-E11000 fallback). Burned sequence numbers on the loser path are an acceptable per-room gap (keyset pagination is gap-tolerant). **Second layer — broadcast de-dup:** the scenario ALSO asserts the _receiver_ gets exactly one `message:new`, but the gRPC `sendMessage` handler fanned out (`message:new` broadcast + `conv:updated` bump + push) once per call; its old `alreadySent` 5s-`createdAt` heuristic can't detect a concurrent dup (the winning row is <5s old), so B saw 5 copies of the one message. Fix: new isolated leaf `apps/chat-service/src/lib/idempotency.ts` — `markIdempotentReplay`/`isIdempotentReplay`, a transient `Symbol` marker (invisible to JSON/spread/Prisma writes). Every dedup return in all 3 message services (pre-send cache/DB hit + dup-catch) is now marked, and the gRPC `sendMessage` handler gates broadcast + bump + push on `!isIdempotentReplay(msg)` (the heuristic was removed), so the single winning insert broadcasts once while the N losers stay silent — yet every caller still gets the same `messageId` ack. **`db-errors.ts` + `idempotency.ts` are committed standalone (isolated leaves, no imports) so HEAD always builds even though the importing service files land via the concurrent reactions-feature commits.** Regression test `apps/chat-service/tests/private/private-message-idempotency.test.ts` (6 service-layer cases: P2002 collapse, raw-E11000 collapse, re-read-miss rethrow, non-dup-error rethrow, happy path, forward collapse). chat-service `tsc --noEmit` clean (0 errors); `*message*` + `service-impl` Jest suites 109/109. **E2E `chaos.duplicateConcurrent` verified GREEN live** (`pnpm -C testing-suite scenarios -- --id chaos.duplicateConcurrent --url http://localhost:3000`): 5 concurrent sends → one `messageId`, no SERVICE_ERROR, B receives exactly one `message:new`.
- **2026-06-15 (media Phase 1 — Telegram-parity foundation; committed `14315e2` + tests):** Additive, backward-compatible. (1) **Single source of truth for message-kind:** new `@aimess/constants` `CONTENT_TYPES` (+ `MEDIA_CONTENT_TYPES`, `COMMUNITY_CONTENT_TYPES`, `isContentType` / `isCommunityContentType` / `contentTypeFromMime`); the 3 chat send validators now import it — **restores `AUDIO`+`GIF` to private/group** (were community-only; that drift was the headline defect). Community derives its lower-case set (excl. `SYSTEM`) from the same list; `MEDIA_MESSAGE_TYPES` gains `AUDIO`. (2) **Widened upload formats** (`apps/media-service/src/config/uploads.ts`): video mkv/webm/avi/m4v, audio m4a/aac/flac, office+text docs (xls/xlsx/ppt/pptx/txt/csv/json/xml). **Not** HEIC or archives/code. **Per-MIME caps:** image 25 MB, gif 30 MB (others → 100 MB category ceiling). (3) **Validation hardening** (`@aimess/storage`): `sanitizeFileName`, `assertExtensionMatchesMime` (new `EXTENSION_MIME_MISMATCH` code), per-MIME cap in `createUploadUrl`, optional `responseContentDisposition` on `createPresignedViewUrl`. (4) **media-service:** optional `originalFileName` on upload-url (sanitized + extension-checked); **forced-download** (`Content-Disposition: attachment`) for non-media keys on download-url via `dispositionForKey`; **`MediaScanner` hook interface** (`src/lib/scanner.ts`, no-op `NoopMediaScanner` — a real engine is Phase 2). Tests green: storage 41/41, media-service upload+serving 22/22; tsc clean on storage/constants/media-service. Full review + phased plan in **`docs/MEDIA_ARCHITECTURE_REVIEW.md`**. Deferred: magic-byte verify + real scanner (P2), thumbnails/transcode + HEIC (P3), POLL/shared-refs/ANIMATION (P4), sticker/GIF provider + per-bucket CDN cutover (P5).
