...
| **Expected Socket/Event** | `community:message:new` to `community:<communityId>` `{ messageId, communityId, roomId, senderId, senderName, senderAvatar, message, contentType, mediaKey, clientMessageId, sentAt(number) }`; `community:updated` to active members' `user:<id>` on `/community` |
